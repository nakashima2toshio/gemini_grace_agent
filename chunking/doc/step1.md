...
- Step1（階層構造化）: 入力テキストを空行（\n\n）を基準に段落単位へ分割。見出しと本文は同一段落として保持。
- Step2（意味的分割）: 段落内を「話題の転換点」で再分割。物理的な改行ではなく、意味のまとまりを基準とする。
- Step3（文脈連続性チェック）: 隣接チャンク間の依存関係を判定。指示語（「この」「それ」）や未定義用語の参照があれば結合、独立して理解可能なら分離。

目的: チャンクサイズが小さすぎると文脈欠落、大きすぎると検索ノイズ増加。この3段階処理により、意味的に完結した適切な粒度のチャンクを生成し、RAG検索の精度向上を図る。
...
|                            2                            | 各ブロックをLLM（Gemini API）に送信<br>・JSON形式（`response_mime_type="application/json"`）<br>                                                              |
| ・Pydanticスキーマ（`response_schema=StructuralResult`） |                                                                                                                                                               |
