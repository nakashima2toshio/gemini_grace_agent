...
def main():
    ...
    group_gen.add_argument("--dataset", type=str, choices=list(DATASET_CONFIGS.keys()), required=True)
    ...
    try:
        ...
        logger.info(f"✅ Q/A生成完了: {result['qa_count']} ペア")
        ...
        if success:
            ...
            logger.info(f"   データ件数  : {result['qa_count']} 件")
            logger.info(f"="*60)
        else:
            logger.error("\n❌ Qdrant登録フェーズで失敗しました。")
