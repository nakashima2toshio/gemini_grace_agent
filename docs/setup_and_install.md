...
**プロジェクト**: gemini_grace_agent（GRACE Agent + RAG システム）  
...
flowchart TB
    ...
    subgraph CLOUD["クラウド API（Google）"]
        GEMINI_LLM["Gemini Chat API\ngemini-2.5-flash-preview-05-20"]
...
| Gemini Chat API | LLM（チャンク分割・Q&A生成・Agent応答） | クラウド（Google） |
...
| API | 用途 | 取得先 |
|-----|------|--------|
| `GOOGLE_API_KEY` | LLM + Embedding（Gemini） | https://aistudio.google.com/ |
| `GEMINI_API_KEY` | LLM + Embedding（代替設定名） | https://aistudio.google.com/ |
| `COHERE_API_KEY` | Rerank（オプション） | https://dashboard.cohere.com/ |

> `GOOGLE_API_KEY` と `GEMINI_API_KEY` は同じキーを設定してください。
...
```bash
git clone https://github.com/nakashima2toshio/gemini_grace_agent.git
...
インストール確認:
```bash
uv run python -c "import streamlit, qdrant_client, google.generativeai; print('OK')"
# → OK
```
...
| パッケージ | バージョン | 用途 |
|----------|----------|------|
| google-generativeai | >=0.8.0 | Gemini API クライアント（LLM + Embedding） |
...
> **`GOOGLE_API_KEY`（または `GEMINI_API_KEY`）は必須です。両方設定することを推奨します。**
...
> Gemini Chat API・Embedding API はクラウド経由のため、ローカルポートは不要です。
...
```bash
# エラー例: ResourceExhausted: 429 Quota exceeded
# 対処: Celery 並列数を下げる
...
│
├── helper/
│   ├── helper_llm.py         # Gemini LLM クライアント
