...
- [ ] #52 単段化（SmartQAResult/SmartQAPair・構造化出力 1 回）＋use_smart_generation 撤去
...
|---|---|---|
| Phase A grace 中核 | 全て実装済（チェック済） | **ほぼ全て未移植**（#56〜#67 が残） |
...
**結論**: gemini は「テストと出力枠は先行して整っているが、GRACE エージェント本体と
Q/A パイプラインの近代化（Phase A/B）が丸ごと残っている」状態。
着手順は **#56（セキュリティ）→ #58（足場）→ Phase A 残 → Phase B → E/F → G/D** を推奨。
